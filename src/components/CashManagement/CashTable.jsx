...
import { cashManagementData } from "../../datas/cashManagementData";
          ...
          <tbody>
            {cashManagementData.map((item) => (
              <tr key={item.id} className="border-b border-gray-300 hover:bg-indigo-50/60 transition-colors">
                ...
                <td className={`px-4 py-3 text-right font-semibold ${item.voucherType === "Phiếu thu" ? "text-green-600" : "text-red-600"}`}>
                  {item.voucherType === "Phiếu thu" ? "+" : "-"}
  ...
  <DescriptionCell text={item.description} />
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-4 py-4 border-t border-gray-300 bg-gray-50">
        <div className="text-sm text-gray-700">
          Tổng số: <span className="font-semibold">{cashManagementData.length}</span>
        </div>
