...
export default function StockTransfer() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        const loadedBranches = Array.isArray(branchData) ? branchData : [];
        setBranches(loadedBranches);
        setProducts(Array.isArray(productData) ? productData : []);
        setFromBranchId(loadedBranches[0]?.id || "");
        setToBranchId(loadedBranches[1]?.id || loadedBranches[0]?.id || "");
      } catch (err) {
        setError(err?.response?.data?.message || "Không thể tải dữ liệu chuyển kho");
      } finally {
        setLoading(false);
      }
    };
  ...
  const totalAmount = items.reduce((sum, item) => {
    ...
    return sum + Number(item.quantity || 0) * Number(product?.costPerUnit || product?.price || 0);
  }, 0);
  ...
  const save = async (complete = false) => {
    try {
      ...
      setMessage(complete ? "Đã hoàn thành phiếu chuyển kho" : "Đã lưu phiếu chuyển kho");
      setItems([emptyLine()]);
      setNote("");
    } catch (err) {
      setError(err?.response?.data?.message || err.message || "Không thể lưu phiếu chuyển kho");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white border-b border-gray-300 px-4 sm:px-6 py-4">
        <h1 className="text-xl sm:text-2xl font-semibold">Thêm mới phiếu chuyển kho</h1>
      </div>

      <div className="p-2 sm:p-4 space-y-4">
        <div className="bg-white border border-gray-300 rounded-lg p-4">
          {loading && <div className="text-sm text-gray-500 mb-3">Đang tải dữ liệu...</div>}
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="border-b border-gray-300 hover:bg-indigo-50/60">
                      ...
                      <td className="p-3 text-right font-semibold">{formatMoney(unitCost * Number(item.quantity || 0))}</td>
        ...
        <div className="bg-white border border-gray-300 rounded-lg">
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between p-4 border-b border-gray-300">
            <div className="text-sm text-gray-500">Chứng từ sẽ được BE tự sinh mã phiếu.</div>
            <div className="text-lg sm:text-xl font-bold self-end">TỔNG TIỀN: {formatMoney(totalAmount)}</div>
          </div>
